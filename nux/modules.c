...
void module_init(unsigned short module_idx) {
	...
	if(I2CEE_exists(i2c_addresses[module_idx])) {
		I2CEE_read(i2c_addresses[module_idx], (unsigned char *) header, sizeof(struct module_info), 0);
		...
		if(crc_sum == header->crc) {
			...
			if(header->profile == PROFILE_UART) {
				...
				I2CEE_read(i2c_addresses[module_idx], (unsigned char *) uart, sizeof(struct uart_profile), sizeof(struct module_info));
				...
				I2CEE_read(i2c_addresses[module_idx], (unsigned char *) relay, sizeof(struct relay_info), sizeof(struct module_info));
